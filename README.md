...
• set total - для установки текста в поле финальной суммы заказа.
...
• а также gett`еры возвращающие необходимые значения.
...
• updateButtonClasses - для установки класса кнопке способа оплаты;
• render - необходимый для корректного рендера FormPayment, наследуемый от render метода родителя (Component) путем super(render).
...
### AppData {
  ...
  Класс имеет методы:
	...
	• resetOrderInfo - для сброса полей форм.